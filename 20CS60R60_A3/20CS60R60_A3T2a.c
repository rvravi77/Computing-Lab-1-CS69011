...
//IF IT IS START EVENT IF WILL GO TO 'if'  PART
   if(temp->se == 's')
   {
       ...
       switch(temp->packagetype)
       {
           ...
           case P1:
            switch(temp->testtype)   
            {
                case ECG:
                 ...
                 break;
                 //WE DO THE SAME THING FOR OTHER CASES AS WELL
                case ECHO:
                if(!isvacant[ECHO])
                 {
                 isvacant[ECHO]=1;  // if ECHO facility is vacant then occupy it
                 comeat[ECHO]=temp->endtime; // Tell next patient to comeat time whenever thsi one finishes
                 push(&pq,'e',convert(temp->endtime),convert(temp->endtime+0.2),P1,ECHO);//push into queue
                 free(temp1);
                 }
                else
                {
                  //if someone comes when a person is inside facility tell him to come at whenever the most current person finishes
                  push(&pq,'s',convert(comeat[ECHO]),convert(comeat[ECHO]+0.2),P1,ECHO);//push into queue
                  comeat[ECHO]=convert(comeat[ECHO]+0.21);// make comeat of ECHO +0.21 such that there is no instant interchange
                  free(temp1);
                }
                break;
                case TREA:
                if(!isvacant[TREA])
                 {
                 isvacant[TREA]=1; // if TREA facility is vacant then occupy it
                 comeat[TREA]=temp->endtime; // Tell next patient to comeat time whenever thsi one finishes
                 push(&pq,'e',convert(temp->endtime),convert(temp->endtime+0.4),P1,0);//push into queue
                 free(temp1);
                 }
                 else
                 {
                     //if someone comes when a person is inside facility tell him to come at whenever the most current person finishes
                  push(&pq,'s',convert(comeat[TREA]),convert(comeat[TREA]+0.4),P1,TREA);//push into queue
                  comeat[TREA]=convert(comeat[TREA]+0.41);// make comeat of TREA +0.41 such that there is no instant interchange
                  free(temp1);
                 }
                 break;
            }
          ...
          case P2:
           if(!isvacant[MRI])
            {
            isvacant[MRI]=1; // if MRI facility is vacant then occupy it
            comeat[MRI]=temp->endtime; // Tell next patient to comeat time whenever thsi one finishes
            push(&pq,'e',convert(temp->endtime),convert(temp->endtime+0.4),P2,0); //push into queue
            free(temp1);
            }
            else{
                //if someone comes when a person is inside facility tell him to come at whenever the most current person finishes
                push(&pq,'s',convert(comeat[MRI]),convert(comeat[MRI]+0.4),P2,MRI);//push into queue
                comeat[MRI]=convert(comeat[MRI]+0.41); // make comeat of MRI +0.41 such that there is no instant interchange
               free(temp1);
            }
            break;
           case P3:
            if(!isvacant[CT])
            {
            isvacant[CT]=1;// if TREA facility is vacant then occupy it
            comeat[CT]=temp->endtime; // Tell next patient to comeat time whenever thsi one finishes
            push(&pq,'e',convert(temp->endtime),convert(temp->endtime+0.3),P3,0); //push into queue
            free(temp1);
            }
            else{
                //if someone comes when a person is inside facility tell him to come at whenever the most current person finishes
                 push(&pq,'s',convert(comeat[CT]),convert(comeat[CT]+0.3),P3,CT); //push into queue
                comeat[CT]=convert(comeat[CT]+0.31); // make comeat of CT +0.31 such that there is no instant interchange
                free(temp1);
            }
            break;
           case P4:
            ...
            isvacant[ECG]=1; // if ECG facility is vacant then occupy it
            comeat[ECG]=temp->endtime; // Tell next patient to comeat time whenever thsi one finishes
            push(&pq,'e',convert(temp->endtime),convert(temp->endtime+0.1),P4,0); //push into queue
            free(temp1);
            }
            else{
                //if someone comes when a person is inside facility tell him to come at whenever the most current person finishes
                 push(&pq,'s',convert(comeat[ECG]),convert(comeat[ECG]+0.1),P4,ECG); //push into queue
                comeat[ECG]=convert(comeat[ECG]+0.11);  // make comeat of ECG +0.11 such that there is no instant interchange
                free(temp1);
            }
            break;
           ...
           if(!isvacant[ECHO])
            {
            //if a P5 patient arrives then push it at the highest priority after P1 and P6
            isvacant[ECHO]=1; // if ECHO facility is vacant then occupy it
            comeat[ECHO]=temp->endtime;// Tell next patient to comeat time whenever thsi one finishes
            push(&pq,'e',convert(temp->endtime),convert(temp->endtime+0.2),P5,0); //push into queue
            free(temp1);
            }
             else{
                  //if someone comes when a person is inside facility tell him to come at whenever the most current person finishes
                 push(&pq,'s',convert(comeat[ECHO]),convert(comeat[ECHO]+0.2),P5,ECHO);//push into queue
                comeat[ECHO]=convert(comeat[ECHO]+0.21); // make comeat of ECHO +0.21 such that there is no instant interchange
                free(temp1);
            }
            break;
           ...
           switch(temp->testtype)   
            {
                ...
                if(!isvacant[ECHO])
                {
                 //if a P6 patient arrives then push it at the highest priority
                 isvacant[ECHO]=1; // if ECHO facility is vacant then occupy it
                 comeat[ECHO]=temp->endtime; // Tell next patient to comeat time whenever thsi one finishes
                 push(&pq,'e',convert(temp->endtime),convert(temp->endtime+0.2),P6,ECHO); //push into queue
                 free(temp1);
                }
                 else{
                //if someone comes when a person is inside facility tell him to come at whenever the most current person finishes
                 push(&pq,'s',convert(comeat[ECHO]),convert(comeat[ECHO]+0.2),P6,ECHO); //push into queue
                comeat[ECHO]=convert(comeat[ECHO]+0.21); // make comeat of ECHO +0.21 such that there is no instant interchange
                free(temp1);
               }
                 break;
                ...
                if(!isvacant[ECG])
                {
                 ...
                 isvacant[ECG]=1; // if ECG facility is vacant then occupy it
                 comeat[ECG]=temp->endtime;// Tell next patient to comeat time whenever thsi one finishes
                 push(&pq,'e',convert(temp->endtime),convert(temp->endtime+0.1),P6,0); //push into queue
                 free(temp1);
                }
                else{
                    //if someone comes when a person is inside facility tell him to come at whenever the most current person finishes
                 push(&pq,'s',convert(comeat[ECG]),convert(comeat[ECG]+0.1),P6,ECG); //push into queue
                comeat[ECG]=convert(comeat[ECG]+0.11); // make comeat of ECG +0.11 such that there is no instant interchange
                free(temp1);
               }
                 break;
   ...
   //calculating theaverage closing time for each facility by the following formula
   // the formula given below considers the start time , the number of patient in each facility
   //average total time required for the test 
   //and the lst comeat time for eachof the facility
