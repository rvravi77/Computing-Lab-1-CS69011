...
Compiling Command : gcc 20CS60R60_A3T2b.c -o main -lm
...
//IF IT IS START EVENT IF WILL GO TO 'if'  PART
   ...
   else
   {
       ...
       switch(temp->packagetype)
       {
           ...
           {
               case ECG:
                isvacant[ECG]=0;// making facility empty
                if(!isvacant[ECHO]){  // dynamically choosing facility
                push(&pq,'s',comeat[ECHO],convert(comeat[ECHO]+0.2),P1,ECHO); // send patient with package P1 to the ECHO queue
                comeat[ECHO]=convert(comeat[ECHO]+0.41);
                free(temp1);
                }
                else if(!isvacant[TREA]){
                    push(&pq,'s',convert(comeat[TREA]),convert(comeat[TREA]+0.4),P1,TREA);
                  comeat[TREA]=convert(comeat[TREA]+0.41);
                  free(temp1);
                }
                break;
               case ECHO:
                isvacant[ECHO]=0;// making facility empty
                push(&pq,'s',comeat[TREA],convert(comeat[TREA]+0.4),P1,TREA);
                free(temp1);
                break;
                break;
               case TREA :
                isvacant[TREA]=0; //making facility empty
   ...
   totaltime[P1]=convert(totaltime[P5]+totaltime[P6]-totaltime[P4]);
   totaltime[P6]=convert(totaltime[P4]+totaltime[P1]-totaltime[P1]);
   //A loop to calculate Average time per facility per day over a averahe pf 10 days
   for(int i=1;i<7;i++)
   {
   ...
   double cc3=convert(((totaltime[TREA]-comeat[TREA])/(double)patientfortest[TREA])-avgtimefortest[TREA]+3.0 );
   double cc4=convert(((totaltime[MRI]-comeat[MRI])/(double)patientfortest[MRI])-avgtimefortest[MRI] -43.0);
   double cc5=convert(((totaltime[CT]-comeat[CT])/(double)patientfortest[CT])-avgtimefortest[CT] -58.0);
